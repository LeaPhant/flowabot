...
let worker_id = crypto.randomBytes(3*4).toString('hex');
let log_as_worker = function(message) {
	helper.log("[worker " + worker_id + "] ", message);
}
async function run_worker_job(data) {
	let { beatmap, start_time, end_time, time_frame, file_path, options, threads, current_frame, size } = data;
		...
		if(beatmap.Replay){
			...
			if(replay_point){
				if(beatmap.Replay.auto !== true){
					...
					ctx.fillRect(canvas.width - 30, keyOverlayTop + KEY_OVERLAY_SIZE + KEY_OVERLAY_PADDING, KEY_OVERLAY_SIZE, KEY_OVERLAY_SIZE);
		...
		while(time < end_time){
			...
			if(options.type === 'gif'){
				for(let i = 0; i < image_data.length; i += 4){
					if(image_data[i + 3] > 0){
						let scale = Math.round(image_data[i] * image_data[i + 3] / 255);
						image_data[i] = scale;
			...
			let abuf = Buffer.from(image_data);
...
ipc.config.id = 'worker-' + worker_id;
ipc.config.retry= 200;
ipc.config.sync = true;
// ipc.config.rawBuffer=true;
// ipc.config.encoding ='base64';
ipc.config.logger = log_as_worker
ipc.config.silent = true;

ipc.connectTo(
	'world',
	ipc.config.socketRoot + ipc.config.appspace + 'world',
	function(){
		// ipc.of.world.on(
		// 	'connect',
		// 	function(){
		...
		ipc.of.world.on(
			...
			function(){
				ipc.log('## connected to world ##', ipc.config.delay);
				ipc.of.world.emit('workerReady', worker_id);
			}
		);
		...
		ipc.of.world.on(
			'setWorker',
			function(data){
				log_as_worker("Setting worker id: " + data);
				worker_id = data;
				// ipc.of.world.emit('app.framedata', {worker_id: worker_id, frame_data: 'sample_data_ignore'});
			}
		)
		...
		ipc.of.world.on('app.terminate', function(data){
			log_as_worker("Terminating...");
			process.exit(0);
		})
